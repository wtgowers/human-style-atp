...
Suppose we have a family $\mathcal{F}$ of proofs we have found, and our current library is $L$. By abstraction we might have identified a result $r$ that we could add to the library. In order to decide whether to add $r$ to the library, we use a criterion inspired by the [https://arxiv.org/abs/2006.08381](DreamCoder paper). We construct the library in such a way as to minimise the entropy
